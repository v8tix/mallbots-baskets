...
import (
	...
	"github.com/v8tix/mallbots-baskets/internal/domain"
)
...
func (h domainHandlers[T]) onBasketStarted(ctx context.Context, event ddd.Event) error {
	...
	return h.publisher.Publish(ctx, pb.BasketAggregateChannel,
		ddd.NewEvent(pb.BasketStartedEvent, &pb.BasketStarted{
			Id:         basket.ID(),
...
func (h domainHandlers[T]) onBasketCanceled(ctx context.Context, event ddd.Event) error {
	basket := event.Payload().(*domain.Basket)
	return h.publisher.Publish(ctx, pb.BasketAggregateChannel,
		ddd.NewEvent(pb.BasketCanceledEvent, &pb.BasketCanceled{
			Id: basket.ID(),
		}),
	)
}
...
func (h domainHandlers[T]) onBasketCheckedOut(ctx context.Context, event ddd.Event) error {
	basket := event.Payload().(*domain.Basket)
	items := make([]*pb.BasketCheckedOut_Item, 0, len(basket.Items))
	for _, item := range basket.Items {
		items = append(items, &pb.BasketCheckedOut_Item{
	...
	return h.publisher.Publish(ctx, pb.BasketAggregateChannel,
		ddd.NewEvent(pb.BasketCheckedOutEvent, &pb.BasketCheckedOut{
			Id:         basket.ID(),
			CustomerId: basket.CustomerID,
